...
export default function ExerciseCard({ index, exercise }) {
  ...
  return (
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
      <div className="flex flex-col sm:flex-row sm:items-center sm:flex-wrap gap-x-4">
        <h4 className="text-3xl hidden sm:inline sm:text-4xl ms:text-5xl font-semibold text-slate-400">
          0{index + 1}
        </h4>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 sm:place-items-center gap-2">
        {["reps", "rest", "tempo"].map((info, infoIndex) => {
          return (
            ...
            >
              ...
              <p className="font-meduim">{exercise[info]}</p>
            </div>
          );
        })}
        <button
        ...
        >
          ...
          <p className="font-meduim">{setsCompleted} / 5</p>
        </button>
      </div>
    </div>
  );
